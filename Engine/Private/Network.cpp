...
const HRESULT CNetwork::Open_Client(const char* serverip, int port, int ip, int id) {
	...
	if (SOCKET_ERROR == connect(_socket, reinterpret_cast<SOCKADDR*>(&socket_addr_in), sizeof(socket_addr_in)))
		return E_FAIL;
	char send_arr[sizeof(_int)];
	ZeroMemory(send_arr, sizeof(_int));
	memcpy(&send_arr[0], &m_iIp, sizeof(_int));
	send(_socket, send_arr, static_cast<int>(sizeof(_int)), 0);
...
void CNetwork::Accept(SOCKET _socket) {
	while (true) {
		SOCKADDR_IN sockaddr_in{};
		_int sockaddr_in_len = sizeof(sockaddr_in);
		SOCKET __socket = accept(_socket, (SOCKADDR*)&sockaddr_in, &sockaddr_in_len);
		if (INVALID_SOCKET == __socket)
			break;
		char buf[sizeof(_int)];
		ZeroMemory(buf, sizeof(_int));
		recv(__socket, buf, sizeof(_int), 0);
		_int Ip = 0;
		memcpy(&Ip, buf, sizeof(_int));
...
void CNetwork::Recv(SOCKET _socket) {
	...
	while (true) {
		char buf[2048] = {};
		_int _return = recv(_socket, buf, 2048, 0);
			...
			while (_index < _return) {
				...
				if (nullptr == buf_arr) {
					...
					if (buf_size_total > _extra + buf_size) {
						...
						_index += static_cast<_int>(buf_size_total) - buf_size;
...
void CNetwork::Set_Send(const _int& id, const string& key, Json::Value& root, const _bool& echo) {
	root["Ip"] = m_iIp;
	root["Id"] = id;
	root["Key"] = key;
	root["Echo"] = echo;
...
const _bool CNetwork::Get_Recv(const _int& id, const string& key, Json::Value& root) {
	Convert_Msg();
...
list<Json::Value>& CNetwork::Get_Recv(const _int& id) {
	return m_arrRecv[id];
}

const _int CNetwork::Get_Id(void) {
	return m_iId++;
}

const _bool& CNetwork::IsServer(void) {
	return m_bIsServer;
}
