...
CFM_Slasher::CFM_Slasher(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, CSlasher_Huntress* const& pSlasher_Huntress)
	: CBase(),
	m_pDevice(pDevice),
	m_pContext(pContext),
	m_pNetwork(pNetwork) {
...
const HRESULT CFM_Slasher::Init(void) {
	...
	m_pFS[EState::IDLE_HUNT] = new CFS_Slasher_Idle_Hunt(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::IDLE_SPIRIT] = new CFS_Slasher_Idle_Spirit(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	...
	m_pFS[EState::RUN_HUNT_FT] = new CFS_Slasher_Run_Hunt(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::RUN_HUNT_BK] = new CFS_Slasher_Run_Hunt(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::RUN_HUNT_RT] = new CFS_Slasher_Run_Hunt(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::RUN_HUNT_LT] = new CFS_Slasher_Run_Hunt(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::RUN_HUNT_FTRT] = new CFS_Slasher_Run_Hunt(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::RUN_HUNT_FTLT] = new CFS_Slasher_Run_Hunt(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::RUN_HUNT_BKRT] = new CFS_Slasher_Run_Hunt(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::RUN_HUNT_BKLT] = new CFS_Slasher_Run_Hunt(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);

	//
	m_pFS[EState::RUN_SPIRIT_FT] = new CFS_Slasher_Run_Spirit(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::RUN_SPIRIT_BK] = new CFS_Slasher_Run_Spirit(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::RUN_SPIRIT_RT] = new CFS_Slasher_Run_Spirit(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::RUN_SPIRIT_LT] = new CFS_Slasher_Run_Spirit(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::RUN_SPIRIT_FTRT] = new CFS_Slasher_Run_Spirit(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::RUN_SPIRIT_FTLT] = new CFS_Slasher_Run_Spirit(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::RUN_SPIRIT_BKRT] = new CFS_Slasher_Run_Spirit(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::RUN_SPIRIT_BKLT] = new CFS_Slasher_Run_Spirit(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	//
	...
	m_pFS[EState::THROW_COOLDOWN] = new CFS_Slasher_Throw_CoolDown(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::THROW_IN] = new CFS_Slasher_Throw_In(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::THROW_SWING] = new CFS_Slasher_Throw_Swing(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);

	//
	m_pFS[EState::SPIRIT_CHARGE] = new CFS_Slasher_Spirit_Charge(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::SPIRIT_OUT] = new CFS_Slasher_Spirit_Out(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::SPIRIT_OUT_BK] = new CFS_Slasher_Spirit_Out(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::SPIRIT_OUT_LT] = new CFS_Slasher_Spirit_Out(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	m_pFS[EState::SPIRIT_OUT_RT] = new CFS_Slasher_Spirit_Out(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
	//
	...
	m_pFS[EState::LOCKER_RELOAD] = new CFS_Slasher_Locker_Reload(m_pDevice, m_pContext, m_pNetwork, m_pSlasher_Huntress);
...
void CFM_Slasher::Update(const _double& dTimeDelta) {
...
void CFM_Slasher::Set_State(const EState& eState) {
	if (m_eState == eState)
		return;
	...
	case CSlasher_Huntress::EType::SLASH: {
		...
		case EState::IDLE_INTRO: {
			m_eState = eState;
			//m_pFS[eState]->Init("BE_DSkeleton_REF.ao|BE_Idle_Slash"); 19
		...
		case EState::IDLE_SLASH: {
			m_eState = eState;
			//m_pFS[eState]->Init("BE_DSkeleton_REF.ao|BE_Idle_Slash"); 19
			m_pSlasher_Huntress->m_pModel2->Set_Anim(19);
			m_pFS[eState]->Init(19);
			break;
		}
		case EState::RUN_SLASH_FT: {
			m_eState = eState;
			//m_pFS[eState]->Init("BE_DSkeleton_REF.ao|BE_RunFT_Slash_FPV"); 31
			if (true == m_pSlasher_Huntress->m_bSlasher)
				m_pFS[eState]->Init(31);
			else
		...
		case EState::RUN_SLASH_BK: {
			m_eState = eState;
			//m_pFS[eState]->Init("BE_DSkeleton_REF.ao|BE_RunBK_Slash_FPV"); 26
			if (true == m_pSlasher_Huntress->m_bSlasher)
		...
		case EState::RUN_SLASH_RT: {
			m_eState = eState;
			//m_pFS[eState]->Init("BE_DSkeleton_REF.ao|BE_RunRT_Slash_FPV"); 39
			if (true == m_pSlasher_Huntress->m_bSlasher)
		...
		case EState::RUN_SLASH_LT: {
			m_eState = eState;
			//m_pFS[eState]->Init("BE_DSkeleton_REF.ao|BE_RunLT_Slash_FPV"); 35
			if (true == m_pSlasher_Huntress->m_bSlasher)
				m_pFS[eState]->Init(35);
			else
				m_pFS[eState]->Init(39);
			break;
		}
		...
		case EState::THROW_COOLDOWN: {
			m_eState = eState;
			m_pSlasher_Huntress->m_pModel2->Set_Anim(61);
			m_pFS[eState]->Init(49);
			break;
		}
		...
		case EState::LOCKER_GRAB: {
			m_eState = eState;
			m_pFS[eState]->Init(15);
			break;
		}
		case EState::LOCKER_RELOAD: {
			m_eState = eState;
			m_pFS[eState]->Init(24);
			break;
		}
		case EState::LOCKER_SEARCH: {
			m_eState = eState;
			m_pFS[eState]->Init(42);
			break;
		}
		case EState::MORI: {
			m_eState = eState;
	...
	case CSlasher_Huntress::EType::HUNT: {
		...
		case EState::THROW_IN: {
			m_eState = eState;
			m_pSlasher_Huntress->m_pModel2->Set_Anim(61);
			m_pFS[eState]->Init(51);
			break;
		}
		case EState::THROW_SWING: {
			m_eState = eState;
			m_pSlasher_Huntress->m_pModel2->Set_Anim(61);
			m_pFS[eState]->Init(52);
			break;
		}
		}
		break;
	...
	case CSlasher_Huntress::EType::SPIRIT: {
		...
		case EState::SPIRIT_CHARGE: {
			m_eState = eState;
			m_pFS[eState]->Init(29);
			break;
		}
		case EState::SPIRIT_OUT: {
			m_eState = eState;
			m_pFS[eState]->Init(31);
			break;
		}
...
const _bool& CFM_Slasher::Get_CamMori(void) {
	return m_bCamMori;
}

void CFM_Slasher::Set_CamMori(const _bool& bCamMori) {
	m_bCamMori = bCamMori;
}

void CFM_Slasher::Update_Network(const string& pKey) {
	...
	if ("Set_T_MoveDir" == pKey) {
		...
		else if (m_eState == EState::IDLE_HUNT) {
			CFS_Slasher_Idle_Hunt* pFS = dynamic_cast<CFS_Slasher_Idle_Hunt*>(m_pFS[EState::IDLE_HUNT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_HUNT_FT) {
			CFS_Slasher_Run_Hunt* pFS = dynamic_cast<CFS_Slasher_Run_Hunt*>(m_pFS[EState::RUN_HUNT_FT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_HUNT_BK) {
			CFS_Slasher_Run_Hunt* pFS = dynamic_cast<CFS_Slasher_Run_Hunt*>(m_pFS[EState::RUN_HUNT_BK]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_HUNT_RT) {
			CFS_Slasher_Run_Hunt* pFS = dynamic_cast<CFS_Slasher_Run_Hunt*>(m_pFS[EState::RUN_HUNT_RT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_HUNT_LT) {
			CFS_Slasher_Run_Hunt* pFS = dynamic_cast<CFS_Slasher_Run_Hunt*>(m_pFS[EState::RUN_HUNT_LT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_HUNT_FTRT) {
			CFS_Slasher_Run_Hunt* pFS = dynamic_cast<CFS_Slasher_Run_Hunt*>(m_pFS[EState::RUN_HUNT_FTRT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_HUNT_FTLT) {
			CFS_Slasher_Run_Hunt* pFS = dynamic_cast<CFS_Slasher_Run_Hunt*>(m_pFS[EState::RUN_HUNT_FTLT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_HUNT_BKRT) {
			CFS_Slasher_Run_Hunt* pFS = dynamic_cast<CFS_Slasher_Run_Hunt*>(m_pFS[EState::RUN_HUNT_BKRT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_HUNT_BKLT) {
			CFS_Slasher_Run_Hunt* pFS = dynamic_cast<CFS_Slasher_Run_Hunt*>(m_pFS[EState::RUN_HUNT_BKLT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::THROW_IN) {
			CFS_Slasher_Throw_In* pFS = dynamic_cast<CFS_Slasher_Throw_In*>(m_pFS[EState::THROW_IN]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::THROW_SWING) {
			CFS_Slasher_Throw_Swing* pFS = dynamic_cast<CFS_Slasher_Throw_Swing*>(m_pFS[EState::THROW_SWING]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::THROW_COOLDOWN) {
			CFS_Slasher_Throw_CoolDown* pFS = dynamic_cast<CFS_Slasher_Throw_CoolDown*>(m_pFS[EState::THROW_COOLDOWN]);
			vDir = pFS->Get_Dir();
		}
	...
	if ("Set_T_NextDir" == pKey) {
		_float3 vDir = {};
		if (m_eState == EState::THROW_IN) {
			CFS_Slasher_Throw_In* pFS = dynamic_cast<CFS_Slasher_Throw_In*>(m_pFS[EState::THROW_IN]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::THROW_SWING) {
	...
	if ("Set_S_MoveDir" == pKey) {
		_float3 vDir = {};
		if (m_eState == EState::IDLE_SPIRIT) {
			CFS_Slasher_Idle_Spirit* pFS = dynamic_cast<CFS_Slasher_Idle_Spirit*>(m_pFS[EState::IDLE_SPIRIT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_SPIRIT_FT) {
			CFS_Slasher_Run_Spirit* pFS = dynamic_cast<CFS_Slasher_Run_Spirit*>(m_pFS[EState::RUN_SPIRIT_FT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_SPIRIT_BK) {
			CFS_Slasher_Run_Spirit* pFS = dynamic_cast<CFS_Slasher_Run_Spirit*>(m_pFS[EState::RUN_SPIRIT_BK]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_SPIRIT_RT) {
			CFS_Slasher_Run_Spirit* pFS = dynamic_cast<CFS_Slasher_Run_Spirit*>(m_pFS[EState::RUN_SPIRIT_RT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_SPIRIT_LT) {
			CFS_Slasher_Run_Spirit* pFS = dynamic_cast<CFS_Slasher_Run_Spirit*>(m_pFS[EState::RUN_SPIRIT_LT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_SPIRIT_FTRT) {
			CFS_Slasher_Run_Spirit* pFS = dynamic_cast<CFS_Slasher_Run_Spirit*>(m_pFS[EState::RUN_SPIRIT_FTRT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_SPIRIT_FTLT) {
			CFS_Slasher_Run_Spirit* pFS = dynamic_cast<CFS_Slasher_Run_Spirit*>(m_pFS[EState::RUN_SPIRIT_FTLT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_SPIRIT_BKRT) {
			CFS_Slasher_Run_Spirit* pFS = dynamic_cast<CFS_Slasher_Run_Spirit*>(m_pFS[EState::RUN_SPIRIT_BKRT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::RUN_SPIRIT_BKLT) {
			CFS_Slasher_Run_Spirit* pFS = dynamic_cast<CFS_Slasher_Run_Spirit*>(m_pFS[EState::RUN_SPIRIT_BKLT]);
			vDir = pFS->Get_Dir();
		}
		else if (m_eState == EState::SPIRIT_OUT) {
			CFS_Slasher_Spirit_Out* pFS = dynamic_cast<CFS_Slasher_Spirit_Out*>(m_pFS[EState::SPIRIT_OUT]);
			vDir = pFS->Get_Dir();
		}
...
void CFM_Slasher::Update_Network(void) {
	CGame_Instance* pGame_Instance = CGame_Instance::Get_Instance();
	...
	for (auto iter = recv.begin(); iter != recv.end();) {
		...
		if ("Set_State" == key) {
			...
			else if (m_eState == EState::THROW_SWING || m_eState == EState::IDLE_HUNT
				|| m_eState == EState::RUN_HUNT_FT || m_eState == EState::RUN_HUNT_BK || m_eState == EState::RUN_HUNT_RT || m_eState == EState::RUN_HUNT_LT
				|| m_eState == EState::RUN_HUNT_FTRT || m_eState == EState::RUN_HUNT_FTLT || m_eState == EState::RUN_HUNT_BKRT || m_eState == EState::RUN_HUNT_BKLT) {
				CFS_Slasher_Throw_Swing* pFS = dynamic_cast<CFS_Slasher_Throw_Swing*>(m_pFS[EState::THROW_SWING]);
				vDir = _float3{ root["Dir"]["x"].asFloat(), root["Dir"]["y"].asFloat(),root["Dir"]["z"].asFloat() };
				pFS->Set_Dir(vDir);
			}
		...
		else if ("Set_S_MoveDir" == key) {
			_float3 vDir = {};
			if (m_eState == EState::IDLE_SPIRIT || m_eState == EState::RUN_SPIRIT_FT || m_eState == EState::RUN_SPIRIT_BK || m_eState == EState::RUN_SPIRIT_RT || m_eState == EState::RUN_SPIRIT_LT
				|| m_eState == EState::RUN_SPIRIT_FTRT || m_eState == EState::RUN_SPIRIT_FTLT || m_eState == EState::RUN_SPIRIT_BKRT || m_eState == EState::RUN_SPIRIT_BKLT) {
				CFS_Slasher_Spirit_Out* pFS = dynamic_cast<CFS_Slasher_Spirit_Out*>(m_pFS[EState::SPIRIT_OUT]);
				vDir = _float3{ root["Dir"]["x"].asFloat(), root["Dir"]["y"].asFloat(),root["Dir"]["z"].asFloat() };
				pFS->Set_Dir(vDir);
			}
