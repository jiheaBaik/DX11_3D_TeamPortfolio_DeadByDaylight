...
const _ubyte CProp_Grass::Update(const _double& dTimeDelta) {
	...
	for (auto& iter : *plistCamper) {
		...
		CCollider* pCamperCollider = dynamic_cast<CCollider*>(pCamper->Get_Component(TEXT("Collider5")));
		...
		if (true == m_pCollider->Is_Collision(pCamperCollider)) {
			...
			if (false == m_bKeep) {
				...
				if (m_bAudio == false) {
					m_pAudio->Play_Sound_Rand(TEXT("sfx_bush"), 7, -1, 0.2f, FMOD_3D, m_pTransform, _float3{ 0.f, 0.f, 0.f }, _float2{ 1.f, 10.f });
					m_bAudio = true;
				}
			}
			else {
