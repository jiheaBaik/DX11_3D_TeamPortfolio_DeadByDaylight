...
CEffect_Hatch* const CEffect_Hatch::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, const _tchar* const& pFilePath) {
	CEffect_Hatch* pInstnace = new CEffect_Hatch(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create(pFilePath)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CEffect* const CEffect_Hatch::Clone(void* const& pArg) {
	CEffect_Hatch* pInstnace = new CEffect_Hatch(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CEffect_Hatch::CEffect_Hatch(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CEffect(pDevice, pContext, pNetwork) {
	XMStoreFloat4x4(&m_ViewMatrix, XMMatrixIdentity());
	XMStoreFloat4x4(&m_ProjMatrix, XMMatrixOrthographicLH((_float)g_iWinSizeX, (_float)g_iWinSizeY, 0.f, 1.f));
}

CEffect_Hatch::CEffect_Hatch(const CEffect_Hatch& rhs)
	: CEffect(rhs),
...
void CEffect_Hatch::Delete(void) {
	__super::Delete();
...
const HRESULT CEffect_Hatch::Init_Create(const _tchar* const& pFilePath) {
	LoadEffect(pFilePath);
	return S_OK;
}

const HRESULT CEffect_Hatch::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
...
const _ubyte CEffect_Hatch::Update(const _double& dTimeDelta) {
	if (m_bCreate) {
		if (m_bStart) {
...
void CEffect_Hatch::Update_Edit(void) {
	m_pTransform->Update_Edit(0);
}

void CEffect_Hatch::Late_Update(const _double& dTimeDelta) {
...
const HRESULT CEffect_Hatch::Render(const CRenderer::EOrder& eOrder) {
	HRESULT hr = S_OK;
...
void CEffect_Hatch::LoadEffect(const _tchar* const& pFilePath) {
...
void CEffect_Hatch::Set_RepeatEnd() {
	m_bEffectEnd = true;
}


void CEffect_Hatch::Set_Pos(_fvector pos){
