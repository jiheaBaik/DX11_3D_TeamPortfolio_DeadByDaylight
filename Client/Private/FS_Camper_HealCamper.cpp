...
CFS_Camper_HealCamper::CFS_Camper_HealCamper(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, CCamper_GamePlay* const& pCamper_GamePlay)
	: CFS(pDevice, pContext, pNetwork) {
	m_pCamper_GamePlay = pCamper_GamePlay;
}

void CFS_Camper_HealCamper::Delete(void) {
	__super::Delete();
	m_pCamper_GamePlay = nullptr;
}

const HRESULT CFS_Camper_HealCamper::Init(_char* ModelName) {
...
const _ubyte  CFS_Camper_HealCamper::Update(const _double& dTimeDelta) {
	...
	if (m_pCamper_GamePlay->m_bControl) {
		...
		if (ANIM_END == m_pCamper_GamePlay->m_pModel->Update_Anim(dTimeDelta, false)) {
			...
			m_pCamper_Target->Get_FM_Camper()->Set_State(CFM_Camper::EState::IDLE);
			m_pCamper_Target->Get_FM_Camper()->Update_Network("Set_State");
