...
list<class CObject*>* CUI_Lobby_Btn_Camper_Menu::s_pMenuItems = nullptr;
_int CUI_Lobby_Btn_Camper_Menu::s_iOnClicked = -1;

CUI_Lobby_Btn_Camper_Menu* const CUI_Lobby_Btn_Camper_Menu::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork) {
	CUI_Lobby_Btn_Camper_Menu* pInstnace = new CUI_Lobby_Btn_Camper_Menu(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create()))
		Safe_Release(pInstnace);
	return pInstnace;
}

CObject* const CUI_Lobby_Btn_Camper_Menu::Clone(void* const& pArg) {
	CUI_Lobby_Btn_Camper_Menu* pInstnace = new CUI_Lobby_Btn_Camper_Menu(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CUI_Lobby_Btn_Camper_Menu::CUI_Lobby_Btn_Camper_Menu(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CUI_Image(pDevice, pContext, pNetwork) {
}

CUI_Lobby_Btn_Camper_Menu::CUI_Lobby_Btn_Camper_Menu(const CUI_Lobby_Btn_Camper_Menu& rhs)
	: CUI_Image(rhs) {
}

void CUI_Lobby_Btn_Camper_Menu::Delete(void) {
	__super::Delete();
//#ifdef _DEBUG
...
const HRESULT CUI_Lobby_Btn_Camper_Menu::Init_Create(void) {
	HRESULT hr = S_OK;
	return hr;
}

const HRESULT CUI_Lobby_Btn_Camper_Menu::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
...
const _ubyte CUI_Lobby_Btn_Camper_Menu::Update(const _double& dTimeDelta) {
	...
	if (__super::IsCollisionStay(m_pCollider, m_pRectTransform)) {
		...
		m_vSpread.y += (_float)dTimeDelta * 0.8f;
		if (1.f <= m_vSpread.y)
			m_vSpread.y = 1.f;
		...
		if (pGame_Instance->Get_Input_MouseButtonUp(CInput::EMouseButton::L)) {
			...
			{
			case Client::CUI_Lobby_Btn_Camper_Menu::EMenuType::ONE: {
				//CCamera_Lobby* pCamera = dynamic_cast<CCamera_Lobby*>(CCamera::Get_Camera(static_cast<_uint>(0)));
				//pCamera->Set_State(CCamera_Lobby::ECAMSTATE::SELECT_CAMPER);
				//pCamera->Set_Group(static_cast<CCamera_Lobby::ECAMSTATE>(m_iGroup), static_cast<CCamera_Lobby::ECAMSTATE>(m_iBtnIdx));
				// 전역 리스트?
				...
				m_vecImages[static_cast<_uint>(ETexType::BRIDGE2)].bRender = true;
																	   ...
																	   break;
			case Client::CUI_Lobby_Btn_Camper_Menu::EMenuType::TWO: {
				this->Set_OnClicked(m_iType);
				this->Set_MenuItmes(m_szMenuItems, m_iGroup);
				m_vecImages[static_cast<_uint>(ETexType::BRIDGE2)].bRender = true;
			}
																	   break;
			case Client::CUI_Lobby_Btn_Camper_Menu::EMenuType::THREE: {
				this->Set_OnClicked(m_iType);
				this->Set_MenuItmes(m_szMenuItems);
				m_vecImages[static_cast<_uint>(ETexType::BRIDGE2)].bRender = true;
			}
																		break;
			case Client::CUI_Lobby_Btn_Camper_Menu::EMenuType::FOUR: {
				this->Set_OnClicked(m_iType);
				this->Set_MenuItmes(m_szMenuItems);
				m_vecImages[static_cast<_uint>(ETexType::BRIDGE2)].bRender = true;
			}
...
void CUI_Lobby_Btn_Camper_Menu::Update_Edit(void) {
	__super::Update_Edit();
	ImGui::Separator();
...
void CUI_Lobby_Btn_Camper_Menu::Late_Update(const _double& dTimeDelta) {
	__super::Late_Update(dTimeDelta);
}

const HRESULT CUI_Lobby_Btn_Camper_Menu::Render(const CRenderer::EOrder& eOrder) {
	HRESULT hr = S_OK;
...
void CUI_Lobby_Btn_Camper_Menu::Set_MenuItmes(const _tchar* pLayer, const _int& iIndex) {
	CGame_Instance* pGame_Instance = CGame_Instance::Get_Instance();
	// off
...
const HRESULT CUI_Lobby_Btn_Camper_Menu::Save_Data(Json::Value& root) {
	__super::Save_Data(root);
	root["Type"] = m_iType;
	this->Save_AddData(L"UI_Lobby_Btn_Camper_Menu");
	return S_OK;
}

void CUI_Lobby_Btn_Camper_Menu::Load_Data(Json::Value& root){
	...
	this->Load_AddData(L"UI_Lobby_Btn_Camper_Menu");
	switch (static_cast<EMenuType>(m_iType))
	{
	case Client::CUI_Lobby_Btn_Camper_Menu::EMenuType::ONE: 
		lstrcpy(m_szMenuItems, L"UI_Lobby_Panel_Camper_Choice");
		break;
	case Client::CUI_Lobby_Btn_Camper_Menu::EMenuType::TWO:
