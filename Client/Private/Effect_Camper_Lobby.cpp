...
CEffect_Camper_Lobby* const CEffect_Camper_Lobby::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, const _tchar* const& pFilePath) {
	CEffect_Camper_Lobby* pInstnace = new CEffect_Camper_Lobby(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create(pFilePath)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CEffect* const CEffect_Camper_Lobby::Clone(void* const& pArg) {
	CEffect_Camper_Lobby* pInstnace = new CEffect_Camper_Lobby(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CEffect_Camper_Lobby::CEffect_Camper_Lobby(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CEffect(pDevice, pContext, pNetwork) {
	XMStoreFloat4x4(&m_ViewMatrix, XMMatrixIdentity());
	XMStoreFloat4x4(&m_ProjMatrix, XMMatrixOrthographicLH((_float)g_iWinSizeX, (_float)g_iWinSizeY, 0.f, 1.f));
}

CEffect_Camper_Lobby::CEffect_Camper_Lobby(const CEffect_Camper_Lobby& rhs)
	: CEffect(rhs),
...
void CEffect_Camper_Lobby::Delete(void) {
	__super::Delete();
...
const HRESULT CEffect_Camper_Lobby::Init_Create(const _tchar* const& pFilePath) {
	LoadEffect(pFilePath);
	return S_OK;
}

const HRESULT CEffect_Camper_Lobby::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
...
const _ubyte CEffect_Camper_Lobby::Update(const _double& dTimeDelta) {
	if (m_bCreate) {
		if (m_bStart) {
...
void CEffect_Camper_Lobby::Update_Edit(void) {
	m_pTransform->Update_Edit(0);
}

void CEffect_Camper_Lobby::Late_Update(const _double& dTimeDelta) {
...
const HRESULT CEffect_Camper_Lobby::Render(const CRenderer::EOrder& eOrder) {
	HRESULT hr = S_OK;
	if (m_pShader == nullptr)
		return hr;
	CCamera_Lobby* pCamera2 = dynamic_cast<CCamera_Lobby*>(CCamera::Get_Camera(static_cast<_uint>(0)));
	if (nullptr != pCamera2)
		if (!pCamera2->Get_CameraCamperRender())
			return S_OK;
...
void CEffect_Camper_Lobby::LoadEffect(const _tchar* const& pFilePath) {
...
void CEffect_Camper_Lobby::Set_Render(const _bool bRender) {
	m_bRender = bRender;
}

const HRESULT CEffect_Camper_Lobby::Save_Data(Json::Value& root) {
	return E_FAIL;
}

void CEffect_Camper_Lobby::Load_Data(Json::Value& root) {
}

void CEffect_Camper_Lobby::Update_Network(const string& pkey) {
}

void CEffect_Camper_Lobby::Update_Network(void) {
}
