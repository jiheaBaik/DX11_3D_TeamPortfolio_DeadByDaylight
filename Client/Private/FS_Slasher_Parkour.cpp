...
CFS_Slasher_Pakour::CFS_Slasher_Pakour(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, CSlasher_Huntress* const& pSlasher_Huntress)
	: CFS(pDevice, pContext, pNetwork) {
	m_pSlasher_Huntress = pSlasher_Huntress;
	//Init();
}

void CFS_Slasher_Pakour::Delete(void) {
	__super::Delete();
}

const HRESULT CFS_Slasher_Pakour::Init(_char* pModelName) {
	m_pSlasher_Huntress->m_pModel->Set_Anim(pModelName);
	m_pSlasher_Huntress->m_pModel_FPV->Set_Anim(pModelName);
	return S_OK;
}

const HRESULT CFS_Slasher_Pakour::Init(_uint iAnimIndex) {
	...
	if (true == m_pSlasher_Huntress->m_bControl) {
		m_pParkour = dynamic_cast<CInter_Parkour*>(m_pSlasher_Huntress->m_pTarObject);
		...
		m_pSlasher_Huntress->m_pTarObject = dynamic_cast<CInter_Parkour*>(pGame_Instance->Get_Object_CloneID(static_cast<_int>(EScene::GAMEPLAY), TEXT("Inter_Parkour"), m_pSlasher_Huntress->m_iTarObjID));
		m_pParkour = dynamic_cast<CInter_Parkour*>(m_pSlasher_Huntress->m_pTarObject);
		m_iParkourDir = m_pSlasher_Huntress->m_iTarObjDir;
	...
	m_pParkourTransform = dynamic_cast<CTransform*>(m_pParkour->Get_Component(TEXT("Transform")));
	...
	case FRONT:
		m_pParkourTransform2 = dynamic_cast<CTransform*>(m_pParkour->Get_Component(TEXT("Transform_S_F")));
		break;
	case BACK:
		m_pParkourTransform2 = dynamic_cast<CTransform*>(m_pParkour->Get_Component(TEXT("Transform_S_B")));
		break;
	}
...
const _ubyte  CFS_Slasher_Pakour::Update(const _double& dTimeDelta) {
	if (true == m_pSlasher_Huntress->m_bControl) {
		...
		if ((_uint)m_pSlasher_Huntress->m_pModel->Get_Anim_TimeAcc() >= 35) {
