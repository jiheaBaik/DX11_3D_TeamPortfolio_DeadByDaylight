...
CSlasher_Trail* const CSlasher_Trail::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork) {
	CSlasher_Trail* pInstnace = new CSlasher_Trail(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create()))
		Safe_Release(pInstnace);
	return pInstnace;
}

CObject* const CSlasher_Trail::Clone(void* const& pArg) {
	CSlasher_Trail* pInstnace = new CSlasher_Trail(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CSlasher_Trail::CSlasher_Trail(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CObject(pDevice, pContext, pNetwork) {
}

CSlasher_Trail::CSlasher_Trail(const CSlasher_Trail& rhs)
	: CObject(rhs) {
}

void CSlasher_Trail::Delete(void) {
	__super::Delete();
...
const HRESULT CSlasher_Trail::Init_Create(void) {
	return S_OK;
}

const HRESULT CSlasher_Trail::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
...
const _ubyte CSlasher_Trail::Update(const _double& dTimeDelta) {
	...
	CTransform* pWeaponTransform = dynamic_cast<CTransform*>(pWeapon_Slash->Get_Component(TEXT("Transform")));
...
void CSlasher_Trail::Update_Edit(void) {
	m_pTransform->Update_Edit(0);	
}

void CSlasher_Trail::Late_Update(const _double& dTimeDelta) {
...
const HRESULT CSlasher_Trail::Render(const CRenderer::EOrder& eOrder) {
	switch (eOrder) {
