...
CSlasher_Spirit_Trail* const CSlasher_Spirit_Trail::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork) {
	CSlasher_Spirit_Trail* pInstnace = new CSlasher_Spirit_Trail(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create()))
		Safe_Release(pInstnace);
	return pInstnace;
}

CObject* const CSlasher_Spirit_Trail::Clone(void* const& pArg) {
	CSlasher_Spirit_Trail* pInstnace = new CSlasher_Spirit_Trail(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CSlasher_Spirit_Trail::CSlasher_Spirit_Trail(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CObject(pDevice, pContext, pNetwork) {
}

CSlasher_Spirit_Trail::CSlasher_Spirit_Trail(const CSlasher_Spirit_Trail& rhs)
	: CObject(rhs) {
}

void CSlasher_Spirit_Trail::Delete(void) {
	__super::Delete();
...
const HRESULT CSlasher_Spirit_Trail::Init_Create(void) {
	return S_OK;
}

const HRESULT CSlasher_Spirit_Trail::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
	...
	m_pTransform->Set_Row(CTransform::ERow::POS, XMVectorSet(0.04f, 0.45f, 0.f, 0.f));

	return hr;
}

const _ubyte CSlasher_Spirit_Trail::Update(const _double& dTimeDelta) {
	...
	CTransform* pWeaponTransform = dynamic_cast<CTransform*>(pWeapon_Spirit->Get_Component(TEXT("Transform")));
...
void CSlasher_Spirit_Trail::Update_Edit(void) {
	m_pTransform->Update_Edit(0);	
}

void CSlasher_Spirit_Trail::Late_Update(const _double& dTimeDelta) {
...
const HRESULT CSlasher_Spirit_Trail::Render(const CRenderer::EOrder& eOrder) {
	switch (eOrder) {
