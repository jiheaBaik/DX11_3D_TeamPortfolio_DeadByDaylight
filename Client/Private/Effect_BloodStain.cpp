...
CEffect_BloodStain* const CEffect_BloodStain::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, const _tchar* const& pFilePath) {
	CEffect_BloodStain* pInstnace = new CEffect_BloodStain(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create(pFilePath)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CObject* const CEffect_BloodStain::Clone(void* const& pArg) {
	CEffect_BloodStain* pInstnace = new CEffect_BloodStain(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CEffect_BloodStain::CEffect_BloodStain(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CObject(pDevice, pContext, pNetwork) {
}

CEffect_BloodStain::CEffect_BloodStain(const CEffect_BloodStain& rhs)
	: CObject(rhs),
	...
	for (auto& iter : m_vecEffect) {
		Safe_AddRef(iter.pModel1);
		Safe_AddRef(iter.pModel2);
		Safe_AddRef(iter.pModel3);
		Safe_AddRef(iter.pModel4);
		Safe_AddRef(iter.pTempTex);
		Safe_AddRef(iter.pDestTex);
	}
}
void CEffect_BloodStain::Delete(void) {
	__super::Delete();
...
const HRESULT CEffect_BloodStain::Init_Create(const _tchar* const& pFilePath) {
	LoadEffect(pFilePath);
	return S_OK;
}

const HRESULT CEffect_BloodStain::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
	...
	for (auto& iter : m_vecEffect) {
		...
		iter.pTransform->Set_Row(CTransform::ERow::RIGHT, XMLoadFloat3(&m_fRight));
		iter.pTransform->Set_Row(CTransform::ERow::UP, XMLoadFloat3(&m_fUp));
		iter.pTransform->Set_Row(CTransform::ERow::LOOK, XMLoadFloat3(&m_fLook));
		iter.pTransform->Set_Row(CTransform::ERow::POS, XMLoadFloat3(&m_fPos));
		iter.pTransform->Set_Scale(XMLoadFloat3(&m_fScale)*0.6f);
	}
...
const _ubyte CEffect_BloodStain::Update(const _double& dTimeDelta) {
	if (m_bCreate) {
		if (m_bStart) {
...
void CEffect_BloodStain::Update_Edit(void) {
	m_pTransform->Update_Edit(0);
}

void CEffect_BloodStain::Late_Update(const _double& dTimeDelta) {
...
const HRESULT CEffect_BloodStain::Render(const CRenderer::EOrder& eOrder) {
	HRESULT hr = S_OK;
	CCamera* pCamera = CCamera::Get_Camera(static_cast<_uint>(0));
...
void CEffect_BloodStain::LoadEffect(const _tchar* const& pFilePath) {
	...
	for (auto& node : root["Node"]) {
		...
		m_fRight = _float3(node["Right"]["x"].asFloat(), node["Right"]["y"].asFloat(), node["Right"]["z"].asFloat());
		m_fUp = _float3(node["Up"]["x"].asFloat(), node["Up"]["y"].asFloat(), node["Up"]["z"].asFloat());
		m_fLook = _float3(node["Look"]["x"].asFloat(), node["Look"]["y"].asFloat(), node["Look"]["z"].asFloat());
		m_fScale = iter.fScale;
		m_fPos = iter.fPos;
...
void CEffect_BloodStain::Update_Network(const string& pkey) {
}

void CEffect_BloodStain::Update_Network(void) {
}
