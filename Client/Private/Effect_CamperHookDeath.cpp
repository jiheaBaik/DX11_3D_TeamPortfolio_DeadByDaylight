...
CEffect_CamperHookDeath* const CEffect_CamperHookDeath::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, const _tchar* const& pFilePath) {
	CEffect_CamperHookDeath* pInstnace = new CEffect_CamperHookDeath(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create(pFilePath)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CObject* const CEffect_CamperHookDeath::Clone(void* const& pArg) {
	CEffect_CamperHookDeath* pInstnace = new CEffect_CamperHookDeath(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CEffect_CamperHookDeath::CEffect_CamperHookDeath(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CObject(pDevice, pContext, pNetwork) {
	XMStoreFloat4x4(&m_ViewMatrix, XMMatrixIdentity());
	XMStoreFloat4x4(&m_ProjMatrix, XMMatrixOrthographicLH((_float)g_iWinSizeX, (_float)g_iWinSizeY, 0.f, 1.f));
}

CEffect_CamperHookDeath::CEffect_CamperHookDeath(const CEffect_CamperHookDeath& rhs)
	: CObject(rhs),
...
void CEffect_CamperHookDeath::Delete(void) {
	__super::Delete();
...
const HRESULT CEffect_CamperHookDeath::Init_Create(const _tchar* const& pFilePath) {
	LoadEffect(pFilePath);
	return S_OK;
}

const HRESULT CEffect_CamperHookDeath::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
...
const _ubyte CEffect_CamperHookDeath::Update(const _double& dTimeDelta) {
	if (m_bCreate) {
		if (m_bStart) {
...
void CEffect_CamperHookDeath::Update_Edit(void) {
	m_pTransform->Update_Edit(0);
}

void CEffect_CamperHookDeath::Late_Update(const _double& dTimeDelta) {
...
const HRESULT CEffect_CamperHookDeath::Render(const CRenderer::EOrder& eOrder) {
	HRESULT hr = S_OK;
	CCamera* pCamera = CCamera::Get_Camera(static_cast<_uint>(0));
...
void CEffect_CamperHookDeath::LoadEffect(const _tchar* const& pFilePath) {
...
void CEffect_CamperHookDeath::Set_RepeatEnd(){
	m_bRepeat = false; 
	for (auto& iter : m_vecEffect){
		iter.bRender = false;
	}
...
void CEffect_CamperHookDeath::Set_Pos(_fvector pos){
