...
CEffect_Free3* const CEffect_Free3::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, const _tchar* const& pFilePath) {
	CEffect_Free3* pInstnace = new CEffect_Free3(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create(pFilePath)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CObject* const CEffect_Free3::Clone(void* const& pArg) {
	CEffect_Free3* pInstnace = new CEffect_Free3(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CEffect_Free3::CEffect_Free3(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CObject(pDevice, pContext, pNetwork) {
	XMStoreFloat4x4(&m_ViewMatrix, XMMatrixIdentity());
	XMStoreFloat4x4(&m_ProjMatrix, XMMatrixOrthographicLH((_float)g_iWinSizeX, (_float)g_iWinSizeY, 0.f, 1.f));
}

CEffect_Free3::CEffect_Free3(const CEffect_Free3& rhs)
	: CObject(rhs),
...
void CEffect_Free3::Delete(void) {
	__super::Delete();
...
const HRESULT CEffect_Free3::Init_Create(const _tchar* const& pFilePath) {
	LoadEffect(pFilePath);
	return S_OK;
}

const HRESULT CEffect_Free3::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
...
const _ubyte CEffect_Free3::Update(const _double& dTimeDelta) {
	if (m_bCreate) {
		if (m_bStart) {
...
void CEffect_Free3::Update_Edit(void) {
	m_pTransform->Update_Edit(0);
}

void CEffect_Free3::Late_Update(const _double& dTimeDelta) {
...
const HRESULT CEffect_Free3::Render(const CRenderer::EOrder& eOrder) {
	HRESULT hr = S_OK;
...
void CEffect_Free3::LoadEffect(const _tchar* const& pFilePath) {
...
void CEffect_Free3::Set_RepeatEnd() {
	m_bEffectEnd = true;
}


void CEffect_Free3::Set_Pos(_fvector pos){
