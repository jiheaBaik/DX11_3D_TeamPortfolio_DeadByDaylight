...
CFS_Slasher_Grab_Parkour_BK::CFS_Slasher_Grab_Parkour_BK(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, CSlasher_Huntress* const& pSlasher_Huntress)
	: CFS(pDevice, pContext, pNetwork) {
	m_pSlasher_Huntress = pSlasher_Huntress;
	//Init();
}

void CFS_Slasher_Grab_Parkour_BK::Delete(void) {
	__super::Delete();
}

const HRESULT CFS_Slasher_Grab_Parkour_BK::Init(_char* pModelName) {
	m_pSlasher_Huntress->m_pModel->Set_Anim(pModelName);
	m_pSlasher_Huntress->m_pModel_FPV->Set_Anim(pModelName);
	return S_OK;
}

const HRESULT CFS_Slasher_Grab_Parkour_BK::Init(_uint iAnimIndex) {
	if (true == m_pSlasher_Huntress->m_bSlasher) {
		...
		m_pSlasher_Huntress->m_pModel->Set_Anim(20);
		m_pSlasher_Huntress->m_pModel_FPV->Set_Anim(20);
	}
	return S_OK;
}

const _ubyte  CFS_Slasher_Grab_Parkour_BK::Update(const _double& dTimeDelta) {
	if (true == m_pSlasher_Huntress->m_bControl) {
		if (m_pSlasher_Huntress->m_pModel_FPV->Get_Anim_TimeAcc() >= 70) {
