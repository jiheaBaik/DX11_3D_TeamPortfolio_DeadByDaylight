...
CUI_Lobby_Panel_Camper* const CUI_Lobby_Panel_Camper::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork) {
	CUI_Lobby_Panel_Camper* pInstnace = new CUI_Lobby_Panel_Camper(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create()))
		Safe_Release(pInstnace);
	return pInstnace;
}

CObject* const CUI_Lobby_Panel_Camper::Clone(void* const& pArg) {
	CUI_Lobby_Panel_Camper* pInstnace = new CUI_Lobby_Panel_Camper(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CUI_Lobby_Panel_Camper::CUI_Lobby_Panel_Camper(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CUI_Image(pDevice, pContext, pNetwork) {
}

CUI_Lobby_Panel_Camper::CUI_Lobby_Panel_Camper(const CUI_Lobby_Panel_Camper& rhs)
	: CUI_Image(rhs) {
}

void CUI_Lobby_Panel_Camper::Delete(void) {
	__super::Delete();
}

const HRESULT CUI_Lobby_Panel_Camper::Init_Create(void) {
	HRESULT hr = S_OK;
	m_iGroup = 3;
	return hr;
}

const HRESULT CUI_Lobby_Panel_Camper::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
	hr |= __super::Init_Clone(/*TEXT("sel_panel")*/);
	return hr;
}

const _ubyte CUI_Lobby_Panel_Camper::Update(const _double& dTimeDelta) {
...
void CUI_Lobby_Panel_Camper::Update_Edit(void) {
	__super::Update_Edit();
	ImGui::Separator();
...
void CUI_Lobby_Panel_Camper::Late_Update(const _double& dTimeDelta) {
	__super::Late_Update(dTimeDelta);
}

const HRESULT CUI_Lobby_Panel_Camper::Render(const CRenderer::EOrder& eOrder) {
	HRESULT hr = S_OK;
	if (!m_bActiveSelf)
		return hr;
...
const HRESULT CUI_Lobby_Panel_Camper::Save_Data(Json::Value& root) {
	__super::Save_Data(root);
	return S_OK;
}

void CUI_Lobby_Panel_Camper::Load_Data(Json::Value& root) {
