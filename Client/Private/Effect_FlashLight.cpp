...
CEffect_FlashLight* const CEffect_FlashLight::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, const _tchar* const& pFilePath) {
	CEffect_FlashLight* pInstnace = new CEffect_FlashLight(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create(pFilePath)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CObject* const CEffect_FlashLight::Clone(void* const& pArg) {
	CEffect_FlashLight* pInstnace = new CEffect_FlashLight(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CEffect_FlashLight::CEffect_FlashLight(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CObject(pDevice, pContext, pNetwork) {
}

CEffect_FlashLight::CEffect_FlashLight(const CEffect_FlashLight& rhs)
	: CObject(rhs),
...
void CEffect_FlashLight::Delete(void) {
	__super::Delete();
...
const HRESULT CEffect_FlashLight::Init_Create(const _tchar* const& pFilePath) {
	LoadEffect(pFilePath);
	return S_OK;
}

const HRESULT CEffect_FlashLight::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
...
const _ubyte CEffect_FlashLight::Update(const _double& dTimeDelta) {
	if (m_bCreate) {
		if (m_bStart) {
...
void CEffect_FlashLight::Update_Edit(void) {
	m_pTransform->Update_Edit(0);
}

void CEffect_FlashLight::Late_Update(const _double& dTimeDelta) {
...
const HRESULT CEffect_FlashLight::Render(const CRenderer::EOrder& eOrder) {
	HRESULT hr = S_OK;
	CCamera* pCamera = CCamera::Get_Camera(static_cast<_uint>(0));
...
void CEffect_FlashLight::LoadEffect(const _tchar* const& pFilePath) {
...
void CEffect_FlashLight::LightOff(){
	for (auto& iter : m_vecEffect) {
		iter.fTime = 0.f;
		iter.bRender = false;
	}
...
void CEffect_FlashLight::LightOn(){
	for (auto& iter : m_vecEffect) {
		iter.bRender = true;
		iter.pTransform->Set_Scale(XMLoadFloat3(&iter.fScale));
		iter.fTime = 0.f;
...
void CEffect_FlashLight::Update_Network(const string& pkey) {
}

void CEffect_FlashLight::Update_Network(void) {
}
