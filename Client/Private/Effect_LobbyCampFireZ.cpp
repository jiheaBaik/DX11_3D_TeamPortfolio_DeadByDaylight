...
CEffect_LobbyCampFireZ* const CEffect_LobbyCampFireZ::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, const _tchar* const& pFilePath) {
	CEffect_LobbyCampFireZ* pInstnace = new CEffect_LobbyCampFireZ(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create(pFilePath)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CObject* const CEffect_LobbyCampFireZ::Clone(void* const& pArg) {
	CEffect_LobbyCampFireZ* pInstnace = new CEffect_LobbyCampFireZ(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CEffect_LobbyCampFireZ::CEffect_LobbyCampFireZ(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CObject(pDevice, pContext, pNetwork) {
	XMStoreFloat4x4(&m_ViewMatrix, XMMatrixIdentity());
	XMStoreFloat4x4(&m_ProjMatrix, XMMatrixOrthographicLH((_float)g_iWinSizeX, (_float)g_iWinSizeY, 0.f, 1.f));
}

CEffect_LobbyCampFireZ::CEffect_LobbyCampFireZ(const CEffect_LobbyCampFireZ& rhs)
	: CObject(rhs),
...
void CEffect_LobbyCampFireZ::Delete(void) {
	__super::Delete();
...
const HRESULT CEffect_LobbyCampFireZ::Init_Create(const _tchar* const& pFilePath) {
	LoadEffect(pFilePath);
	return S_OK;
}

const HRESULT CEffect_LobbyCampFireZ::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
	...
	m_bCreate = true;
	m_bStart = true;
	//m_pAudio->Play_Sound_Rand(TEXT("sfx_campfire_menu_loop"), 2, -1, 1.f, FMOD_3D | FMOD_LOOP_NORMAL, m_pTransform, _float3{ 0.f, 0.f, 0.f }, _float2{ 1.f, 10.f });
	//m_pAudio->Play_Sound_Rand(TEXT("sfx_campfire_menu_loop"), 2, -1, 1.f, FMOD_2D | FMOD_LOOP_NORMAL);
	m_pTarget_Depth = CTarget_Mgr::Get_Instance()->Get_Target(TEXT("Depth"));
	return hr;
}

const _ubyte CEffect_LobbyCampFireZ::Update(const _double& dTimeDelta) {
	if (m_bCreate) {
		if (m_bStart) {
			m_dMainTime += dTimeDelta;
			if (m_bSoundFirst) {
				m_pAudio->Play_Sound_Rand(TEXT("sfx_campfire_menu_loop"), 2, -1, 0.4f, FMOD_3D | FMOD_LOOP_NORMAL, m_pTransform, _float3{ 0.f, 0.f, 0.f }, _float2{ 1.f, 8.f });
				m_bSoundFirst = false;
			}
			for (auto& iter : m_vecEffect) {
...
void CEffect_LobbyCampFireZ::Update_Edit(void) {
	m_pTransform->Update_Edit(0);
}

void CEffect_LobbyCampFireZ::Late_Update(const _double& dTimeDelta) {
...
const HRESULT CEffect_LobbyCampFireZ::Render(const CRenderer::EOrder& eOrder) {
	HRESULT hr = S_OK;
	CCamera* pCamera = CCamera::Get_Camera(static_cast<_uint>(0));
...
void CEffect_LobbyCampFireZ::LoadEffect(const _tchar* const& pFilePath) {
...
const HRESULT CEffect_LobbyCampFireZ::Save_Data(Json::Value& root) {
	m_pTransform->Save_Data(root["Transform"]);
	return S_OK;
}

void CEffect_LobbyCampFireZ::Load_Data(Json::Value& root) {
	m_pTransform->Load_Data(root["Transform"]);
}

void CEffect_LobbyCampFireZ::Update_Network(const string& pkey) {
}

void CEffect_LobbyCampFireZ::Update_Network(void) {
}
