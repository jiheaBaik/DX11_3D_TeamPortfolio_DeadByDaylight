...
CFS_Camper_Closet_UnHide::CFS_Camper_Closet_UnHide(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, CCamper_GamePlay* const& pCamper_GamePlay)
	: CFS(pDevice, pContext, pNetwork) {
	m_pCamper_GamePlay = pCamper_GamePlay;
}

void CFS_Camper_Closet_UnHide::Delete(void) {
	__super::Delete();
	m_pCamper_GamePlay = nullptr;
}

const HRESULT CFS_Camper_Closet_UnHide::Init(_char* ModelName) {
	...
	m_pCamper_GamePlay->m_pTransform->Set_Row(CTransform::ERow::POS, m_pLockerTarnsform->Get_Row(CTransform::ERow::POS));
...
const _ubyte  CFS_Camper_Closet_UnHide::Update(const _double& dTimeDelta) {
