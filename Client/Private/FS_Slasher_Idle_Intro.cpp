#include "pch.h"
#include "FS_Slasher_Idle_Intro.h"
#include "Slasher_Huntress.h"
#include "UI_Play_Icon_Hud.h"
#include "UI_Play_Icon_Passive.h"

CFS_Slasher_Idle_Intro::CFS_Slasher_Idle_Intro(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, CSlasher_Huntress* const& pSlasher_Huntress)
	: CFS(pDevice, pContext, pNetwork) {
	m_pSlasher_Huntress = pSlasher_Huntress;
}

void CFS_Slasher_Idle_Intro::Delete(void) {
	__super::Delete();
}

const HRESULT CFS_Slasher_Idle_Intro::Init(_char* pModelName) {
	m_pSlasher_Huntress->m_pModel->Set_Anim(pModelName);
	m_pSlasher_Huntress->m_pModel_FPV->Set_Anim(pModelName);

	return S_OK;
}

const HRESULT CFS_Slasher_Idle_Intro::Init(_uint iAnimIndex) {
	if (true == m_pSlasher_Huntress->m_bSlasher) {
		m_pSlasher_Huntress->m_pModel_FPV->Set_Anim(iAnimIndex);
		m_pSlasher_Huntress->m_pModel->Set_Anim(iAnimIndex);
	}
	else {
		m_pSlasher_Huntress->m_pModel_FPV->Set_Anim(25);
		m_pSlasher_Huntress->m_pModel->Set_Anim(25);
	}

	return S_OK;
}

const _ubyte  CFS_Slasher_Idle_Intro::Update(const _double& dTimeDelta) {
	if (true == m_pSlasher_Huntress->m_bControl) {
		if (false == m_bIntroUI) {
			m_bIntroUI = true;
			CGame_Instance* pGame_Instance = CGame_Instance::Get_Instance();
			// set ui
			dynamic_cast<CUI_Play_Icon_Passive*>(pGame_Instance->Get_Object_Clone(static_cast<_ubyte>(EScene::GAMEPLAY), L"UI_Play_Icon_Passive"))->Set_FontCnt(m_pSlasher_Huntress->m_iWeaponCount, false);
			if (0 < m_pSlasher_Huntress->m_iWeaponCount) //  ? passive ready
				dynamic_cast<CUI_Play_Icon_Hud*>(pGame_Instance->Get_Object_Clone(static_cast<_ubyte>(EScene::GAMEPLAY), L"UI_Play_Icon_Hud"))->Set_HudOn(static_cast<_uint>(CUI_Play_Icon_Hud::ESlasher::PASSIVE1), CUI_Play_Icon_Hud::EKey::MOUSE_R, CUI_Play_Icon_Hud::EDiv::CENTER, true);
		}
		// Anim
		m_pSlasher_Huntress->m_pModel->Update_Anim(dTimeDelta, true);
		m_pSlasher_Huntress->m_pModel_FPV->Update_Anim(dTimeDelta, true);
	}
	else {
		// Anim
		m_pSlasher_Huntress->m_pModel->Update_Anim(dTimeDelta, true);
		m_pSlasher_Huntress->m_pModel_FPV->Update_Anim(dTimeDelta, true);
	}
	return CFM_Slasher::EState::END;
}
