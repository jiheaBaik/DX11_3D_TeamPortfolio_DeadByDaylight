...
CEffect_LogoParticle* const CEffect_LogoParticle::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, const _tchar* const& pFilePath) {
	CEffect_LogoParticle* pInstnace = new CEffect_LogoParticle(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create(pFilePath)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CObject* const CEffect_LogoParticle::Clone(void* const& pArg) {
	CEffect_LogoParticle* pInstnace = new CEffect_LogoParticle(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CEffect_LogoParticle::CEffect_LogoParticle(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CUI(pDevice, pContext, pNetwork) {
	XMStoreFloat4x4(&m_ViewMatrix, XMMatrixIdentity());
	XMStoreFloat4x4(&m_ProjMatrix, XMMatrixOrthographicLH((_float)g_iWinSizeX, (_float)g_iWinSizeY, 0.f, 1.f));
}

CEffect_LogoParticle::CEffect_LogoParticle(const CEffect_LogoParticle& rhs)
	: CUI(rhs),
...
void CEffect_LogoParticle::Delete(void) {
	__super::Delete();
...
const HRESULT CEffect_LogoParticle::Init_Create(const _tchar* const& pFilePath) {
	LoadEffect(pFilePath);
	return S_OK;
}

const HRESULT CEffect_LogoParticle::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
	...
	for (auto& iter : m_vecEffect) {
		...
		if (iter.bRandPos) {
			iter.fPos.x = ((iter.fRandPos[1].x - iter.fRandPos[0].x) * rand() / RAND_MAX + iter.fRandPos[0].x) + 750.f - g_iWinSizeX * 0.5f;
			iter.fPos.y = ((iter.fRandPos[1].y - iter.fRandPos[0].y) * rand() / RAND_MAX + iter.fRandPos[0].y) * g_iWinSizeY / 900.f;
		...
		/*iter.fPos.x = (iter.fPos.x / 1600.f) * g_iWinSizeX;
		iter.fPos.y = (iter.fPos.y / 900.f) * g_iWinSizeY;
		iter.vAccel.x = (iter.vAccel.x / 1600.f) * g_iWinSizeX;
		iter.vAccel.y = (iter.vAccel.y / 900.f) * g_iWinSizeY;
		iter.vSetAccel.x = (iter.vSetAccel.x / 1600.f) * g_iWinSizeX;
		iter.vSetAccel.y = (iter.vSetAccel.y / 900.f) * g_iWinSizeY;
		iter.fResist.y = iter.fResist.y / 900.f * g_iWinSizeY;
		iter.fResistP = iter.fResistP / 900.f * g_iWinSizeY;
...
const _ubyte CEffect_LogoParticle::Update(const _double& dTimeDelta) {
	if (m_bCreate) {
		if (m_bStart) {
			...
			for (auto& iter : m_vecEffect) {
				if (m_bRepeat && (m_dMainTime > m_fMainEndTime)) {
					if (!m_bProgressLoop) {
						...
						if (iter.bRandPos) {
							iter.fPos.x = ((iter.fRandPos[1].x - iter.fRandPos[0].x) * rand() / RAND_MAX + iter.fRandPos[0].x) + 800.f - g_iWinSizeX * 0.5f;
							iter.fPos.y = ((iter.fRandPos[1].y - iter.fRandPos[0].y) * rand() / RAND_MAX + iter.fRandPos[0].y) * g_iWinSizeY / 900.f;
							iter.fPos.z = (iter.fRandPos[1].z - iter.fRandPos[0].z) * rand() / RAND_MAX + iter.fRandPos[0].z;
						...
						if (iter.bRandAccel) {
							iter.vSetAccel.x = ((iter.vRandAccel[1].x - iter.vRandAccel[0].x) * rand() / RAND_MAX + iter.vRandAccel[0].x) * g_iWinSizeX / 1600.f;
							iter.vSetAccel.y = ((iter.vRandAccel[1].y - iter.vRandAccel[0].y) * rand() / RAND_MAX + iter.vRandAccel[0].y) * g_iWinSizeY / 900.f;
							iter.vSetAccel.z = (iter.vRandAccel[1].z - iter.vRandAccel[0].z) * rand() / RAND_MAX + iter.vRandAccel[0].z;
						}
						iter.vAccel = iter.vSetAccel;

						if (iter.bRandResist) {
							iter.fResist.y = ((iter.fRandResist[1].y - iter.fRandResist[0].y) * rand() / RAND_MAX + iter.fRandResist[0].y) * ((g_iWinSizeX * g_iWinSizeY) / (1600.f * 900.f));
						}
...
void CEffect_LogoParticle::Update_Edit(void) {
	m_pTransform->Update_Edit(0);
	ImGui::InputInt("depth", &m_iDepth);
}

void CEffect_LogoParticle::Late_Update(const _double& dTimeDelta) {
	if (nullptr == m_pRenderer)
		return;
	m_pRenderer->Add_Object(CRenderer::EOrder::UI, this);
}

const HRESULT CEffect_LogoParticle::Render(const CRenderer::EOrder& eOrder) {
	HRESULT hr = S_OK;
	...
	for (auto& iter : m_vecEffect) {
		if (iter.bRender) {
			...
			case CRenderer::EOrder::NONLIGHT: {
				...
				hr |= m_pShader->Set_RawValue("g_PlusColor", &iter.fPlusColor, sizeof(_float4));
				hr |= m_pShader->Set_RawValue("g_fGlow_Buffer", &iter.fGlow_Buffer, sizeof(_float2));
				...
				hr |= m_pShader->Set_RawValue("flowUV", &iter.fFlowUV, sizeof(_float2));
...
void CEffect_LogoParticle::LoadEffect(const _tchar* const& pFilePath) {
...
const HRESULT CEffect_LogoParticle::Save_Data(Json::Value& root) {
	m_pTransform->Save_Data(root["Transform"]);
	root["Depth"] = m_iDepth;
	return S_OK;
}

void CEffect_LogoParticle::Load_Data(Json::Value& root) {
