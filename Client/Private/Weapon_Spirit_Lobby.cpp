...
CWeapon_Spirit_Lobby* const CWeapon_Spirit_Lobby::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork) {
	CWeapon_Spirit_Lobby* pInstance = new CWeapon_Spirit_Lobby(pDevice, pContext, pNetwork);
	if (FAILED(pInstance->Init_Create()))
		Safe_Release(pInstance);
	return pInstance;
}

CObject* const CWeapon_Spirit_Lobby::Clone(void* const& pArg) {
	CWeapon_Spirit_Lobby* pInstance = new CWeapon_Spirit_Lobby(*this);
	if (FAILED(pInstance->Init_Clone(pArg)))
		Safe_Release(pInstance);
	return pInstance;
}

CWeapon_Spirit_Lobby::CWeapon_Spirit_Lobby(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CObject(pDevice, pContext, pNetwork) {
}

CWeapon_Spirit_Lobby::CWeapon_Spirit_Lobby(const CWeapon_Spirit_Lobby& rhs)
	: CObject(rhs) {
}

void CWeapon_Spirit_Lobby::Delete(void) {
	__super::Delete();
...
const HRESULT CWeapon_Spirit_Lobby::Init_Create(void) {
	return S_OK;
}

const HRESULT CWeapon_Spirit_Lobby::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
	...
	hr |= __super::Add_Component(static_cast<_ubyte>(EScene::LOBBY), TEXT("Model_Weapon_Spirit_Lobby"), TEXT("Model"), reinterpret_cast<CComponent*&>(m_pModel));
...
const _ubyte CWeapon_Spirit_Lobby::Update(const _double& dTimeDelta) {
...
void CWeapon_Spirit_Lobby::Update_Edit(void) {
...
void CWeapon_Spirit_Lobby::Late_Update(const _double& dTimeDelta) {
	CGame_Instance* pGame_Instance = CGame_Instance::Get_Instance();
	...
	if (pSlasher->Get_State() == CSlasher_Spirit_Lobby::ESTATE::IDLE || pSlasher->Get_State() == CSlasher_Spirit_Lobby::ESTATE::IDLE_THROW)
...
const HRESULT CWeapon_Spirit_Lobby::Render(const CRenderer::EOrder& eOrder) {
	CGame_Instance* pGame_Instance = CGame_Instance::Get_Instance();
	switch (eOrder) {
...
void CWeapon_Spirit_Lobby::Set_Dissolve(const _float& m_fGoal) {
	m_fGoalDissolveLength = m_fGoal;
}

const HRESULT CWeapon_Spirit_Lobby::Save_Data(Json::Value& root) {
	return S_OK;
}

void CWeapon_Spirit_Lobby::Load_Data(Json::Value& root) {
