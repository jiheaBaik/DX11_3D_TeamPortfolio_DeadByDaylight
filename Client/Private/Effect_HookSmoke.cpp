...
CEffect_HookSmoke* const CEffect_HookSmoke::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, const _tchar* const& pFilePath) {
	CEffect_HookSmoke* pInstnace = new CEffect_HookSmoke(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create(pFilePath)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CEffect* const CEffect_HookSmoke::Clone(void* const& pArg) {
	CEffect_HookSmoke* pInstnace = new CEffect_HookSmoke(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CEffect_HookSmoke::CEffect_HookSmoke(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CEffect(pDevice, pContext, pNetwork) {
	XMStoreFloat4x4(&m_ViewMatrix, XMMatrixIdentity());
	XMStoreFloat4x4(&m_ProjMatrix, XMMatrixOrthographicLH((_float)g_iWinSizeX, (_float)g_iWinSizeY, 0.f, 1.f));
}

CEffect_HookSmoke::CEffect_HookSmoke(const CEffect_HookSmoke& rhs)
	: CEffect(rhs),
...
void CEffect_HookSmoke::Delete(void) {
	__super::Delete();
...
const HRESULT CEffect_HookSmoke::Init_Create(const _tchar* const& pFilePath) {
	LoadEffect(pFilePath);
	return S_OK;
}

const HRESULT CEffect_HookSmoke::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
...
const _ubyte CEffect_HookSmoke::Update(const _double& dTimeDelta) {
	if (m_bCreate) {
		if (m_bStart) {
...
void CEffect_HookSmoke::Update_Edit(void) {
	m_pTransform->Update_Edit(0);
}

void CEffect_HookSmoke::Late_Update(const _double& dTimeDelta) {
...
const HRESULT CEffect_HookSmoke::Render(const CRenderer::EOrder& eOrder) {
	HRESULT hr = S_OK;
...
void CEffect_HookSmoke::LoadEffect(const _tchar* const& pFilePath) {
...
void CEffect_HookSmoke::Set_RepeatEnd() {
	m_bEffectEnd = true;
}


void CEffect_HookSmoke::Set_Pos(_fvector pos){
