...
CEffect_FallingLeaf* const CEffect_FallingLeaf::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, const _tchar* const& pFilePath) {
	CEffect_FallingLeaf* pInstnace = new CEffect_FallingLeaf(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create(pFilePath)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CEffect* const CEffect_FallingLeaf::Clone(void* const& pArg) {
	CEffect_FallingLeaf* pInstnace = new CEffect_FallingLeaf(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CEffect_FallingLeaf::CEffect_FallingLeaf(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CEffect(pDevice, pContext, pNetwork) {
	XMStoreFloat4x4(&m_ViewMatrix, XMMatrixIdentity());
	XMStoreFloat4x4(&m_ProjMatrix, XMMatrixOrthographicLH((_float)g_iWinSizeX, (_float)g_iWinSizeY, 0.f, 1.f));
}

CEffect_FallingLeaf::CEffect_FallingLeaf(const CEffect_FallingLeaf& rhs)
	: CEffect(rhs),
...
void CEffect_FallingLeaf::Delete(void) {
	__super::Delete();
...
const HRESULT CEffect_FallingLeaf::Init_Create(const _tchar* const& pFilePath) {
	LoadEffect(pFilePath);
	return S_OK;
}

const HRESULT CEffect_FallingLeaf::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
...
const _ubyte CEffect_FallingLeaf::Update(const _double& dTimeDelta) {
	if (m_bCreate) {
		if (m_bStart) {
...
void CEffect_FallingLeaf::Update_Edit(void) {
	m_pTransform->Update_Edit(0);
}

void CEffect_FallingLeaf::Late_Update(const _double& dTimeDelta) {
...
const HRESULT CEffect_FallingLeaf::Render(const CRenderer::EOrder& eOrder) {
	HRESULT hr = S_OK;
	CCamera* pCamera = CCamera::Get_Camera(static_cast<_uint>(0));
...
void CEffect_FallingLeaf::LoadEffect(const _tchar* const& pFilePath) {
...
const HRESULT CEffect_FallingLeaf::Save_Data(Json::Value& root){
	m_pTransform->Save_Data(root["Transform"]);
	return S_OK;
}

void CEffect_FallingLeaf::Load_Data(Json::Value& root){
