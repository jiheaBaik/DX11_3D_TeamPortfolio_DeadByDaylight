...
CFS_Slasher_Stun_Drop_Out::CFS_Slasher_Stun_Drop_Out(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, CSlasher_Huntress* const& pSlasher_Huntress)
	: CFS(pDevice, pContext, pNetwork) {
	m_pSlasher_Huntress = pSlasher_Huntress;
	//Init();
}

void CFS_Slasher_Stun_Drop_Out::Delete(void) {
	__super::Delete();
}

const HRESULT CFS_Slasher_Stun_Drop_Out::Init(_char* pModelName) {
	m_pSlasher_Huntress->m_pModel->Set_Anim(pModelName);
	m_pSlasher_Huntress->m_pModel_FPV->Set_Anim(pModelName);
	return S_OK;
}

const HRESULT CFS_Slasher_Stun_Drop_Out::Init(_uint iAnimIndex) {
	if (true == m_pSlasher_Huntress->m_bSlasher) {
		...
		m_pSlasher_Huntress->m_pModel->Set_Anim(43);
		m_pSlasher_Huntress->m_pModel_FPV->Set_Anim(43);
	}

	return S_OK;
}

const _ubyte  CFS_Slasher_Stun_Drop_Out::Update(const _double& dTimeDelta) {
