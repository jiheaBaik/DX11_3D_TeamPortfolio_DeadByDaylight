...
CEffect_DrumFire* const CEffect_DrumFire::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork, const _tchar* const& pFilePath) {
	CEffect_DrumFire* pInstnace = new CEffect_DrumFire(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create(pFilePath)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CObject* const CEffect_DrumFire::Clone(void* const& pArg) {
	CEffect_DrumFire* pInstnace = new CEffect_DrumFire(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CEffect_DrumFire::CEffect_DrumFire(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CObject(pDevice, pContext, pNetwork) {
	XMStoreFloat4x4(&m_ViewMatrix, XMMatrixIdentity());
	XMStoreFloat4x4(&m_ProjMatrix, XMMatrixOrthographicLH((_float)g_iWinSizeX, (_float)g_iWinSizeY, 0.f, 1.f));
}

CEffect_DrumFire::CEffect_DrumFire(const CEffect_DrumFire& rhs)
	: CObject(rhs),
...
void CEffect_DrumFire::Delete(void) {
	__super::Delete();
...
const HRESULT CEffect_DrumFire::Init_Create(const _tchar* const& pFilePath) {
	LoadEffect(pFilePath);
	return S_OK;
}

const HRESULT CEffect_DrumFire::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
	...
	//m_pAudio->Play_Sound_Rand(TEXT("sfx_campfire_loop"), 2, -1, 1.f, FMOD_3D | FMOD_LOOP_NORMAL, m_pTransform, _float3{ 0.f, 0.f, 0.f }, _float2{ 1.f, 10.f });
	m_bCreate = true;
	m_bStart = true;
...
const _ubyte CEffect_DrumFire::Update(const _double& dTimeDelta) {
	if (m_bCreate) {
		if (m_bStart) {
			if (m_bSoundFirst) {
				m_pAudio->Play_Sound_Rand(TEXT("sfx_campfire_loop"), 2, -1, 0.2f, FMOD_3D | FMOD_LOOP_NORMAL, m_pTransform, _float3{ 0.f, 0.f, 0.f }, _float2{ 1.f, 5.f });
				m_bSoundFirst = false;
			}
...
void CEffect_DrumFire::Update_Edit(void) {
	m_pTransform->Update_Edit(0);
}

void CEffect_DrumFire::Late_Update(const _double& dTimeDelta) {
...
const HRESULT CEffect_DrumFire::Render(const CRenderer::EOrder& eOrder) {
	HRESULT hr = S_OK;
	CCamera* pCamera = CCamera::Get_Camera(static_cast<_uint>(0));
...
void CEffect_DrumFire::LoadEffect(const _tchar* const& pFilePath) {
...
const HRESULT CEffect_DrumFire::Save_Data(Json::Value& root) {
	m_pTransform->Save_Data(root["Transform"]);
	return S_OK;
}

void CEffect_DrumFire::Load_Data(Json::Value& root) {
