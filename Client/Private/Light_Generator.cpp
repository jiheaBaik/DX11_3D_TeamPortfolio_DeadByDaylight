...
CLight_Generator* const CLight_Generator::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork) {
	CLight_Generator* pInstnace = new CLight_Generator(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create()))
		Safe_Release(pInstnace);
	return pInstnace;
}

CObject* const CLight_Generator::Clone(void* const& pArg) {
	CLight_Generator* pInstnace = new CLight_Generator(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CLight_Generator::CLight_Generator(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CLight(pDevice, pContext, pNetwork) {
}

CLight_Generator::CLight_Generator(const CLight_Generator& rhs)
	: CLight(rhs) {
}

void CLight_Generator::Delete(void) {
	__super::Delete();
	Safe_Release(m_pRenderer);
	Safe_Release(m_pCollider);
}

const HRESULT CLight_Generator::Init_Create(void) {
	return S_OK;
}

const HRESULT CLight_Generator::Init_Clone(void* const& pArg) {
	HRESULT hr = S_OK;
...
const _ubyte CLight_Generator::Update(const _double& dTimeDelta) {
	m_pCollider->Update(m_pTransform->Get_Matrix());
	if (m_bOnOff) {
		m_dOffTime += dTimeDelta;
		m_sDesc.vDiffuse.y += static_cast<_float>(dTimeDelta) * 5.f;
		m_sDesc.vDiffuse.z -= static_cast<_float>(dTimeDelta) * 10.f;		
		if (m_sDesc.vDiffuse.z < 0.f)
			m_sDesc.vDiffuse.z = 0.f;
		
		m_sDesc.vAmbient.y += static_cast<_float>(dTimeDelta) * 5.f;
		m_sDesc.vAmbient.z -= static_cast<_float>(dTimeDelta) * 10.f;
		if (m_sDesc.vAmbient.z < 0.f)
			m_sDesc.vAmbient.z = 0.f;

		m_sDesc.fRange -= static_cast<_float>(dTimeDelta) * 5.f;
		if (m_sDesc.fRange < 0.f) {
			m_sDesc.fRange = 0.f;
			m_dOffTime = 0.;
			m_bOnOff = false;
			return OBJ_ERASE;
...
void CLight_Generator::Late_Update(const _double& dTimeDelta) {	
	m_pRenderer->Add_Object(CRenderer::EOrder::NONLIGHT, this);
}

const HRESULT CLight_Generator::Render(const CRenderer::EOrder& eOrder) {
	CCamera* pCamera = CCamera::Get_Camera(static_cast<_uint>(0));
	//m_pCollider->Render(pCamera);
	return S_OK;
}
const HRESULT CLight_Generator::Save_Data(Json::Value& root) {
...
void CLight_Generator::Load_Data(Json::Value& root) {
...
void CLight_Generator::Set_LightOnPos(_fvector pos, CLight::SDesc sDesc){
