...
class CEffect_Hatch final : public CEffect {
	struct DirKeyFrame {
		_float	fDirTime = 0.f;
...
public:
	static CEffect_Hatch* const Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork, const _tchar* const& pFilePath = nullptr);
	virtual CEffect* const Clone(void* const& pArg = nullptr) override;
private:
	explicit CEffect_Hatch(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork);
	explicit CEffect_Hatch(const CEffect_Hatch& rhs);
	virtual ~CEffect_Hatch(void) override = default;
