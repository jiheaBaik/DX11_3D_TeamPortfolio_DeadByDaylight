...
class CEffect_Camper_Lobby final : public CEffect {
	struct DirKeyFrame {
		_float	fDirTime = 0.f;
...
public:
	static CEffect_Camper_Lobby* const Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork, const _tchar* const& pFilePath = nullptr);
	virtual CEffect* const Clone(void* const& pArg = nullptr) override;
private:
	explicit CEffect_Camper_Lobby(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork);
	explicit CEffect_Camper_Lobby(const CEffect_Camper_Lobby& rhs);
	virtual ~CEffect_Camper_Lobby(void) override = default;
...
public:
	...
	void Set_Render(const _bool bRender);
	const HRESULT Save_Data(Json::Value& root);
	void Load_Data(Json::Value& root);
