...
class CEffect_Blood_Free final : public CObject {
	struct EffectModel {
...
public:
	static CEffect_Blood_Free* const Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork);
	virtual CObject* const Clone(void* const& pArg = nullptr) override;
private:
	explicit CEffect_Blood_Free(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork);
	explicit CEffect_Blood_Free(const CEffect_Blood_Free& rhs);
	virtual ~CEffect_Blood_Free(void) override = default;
