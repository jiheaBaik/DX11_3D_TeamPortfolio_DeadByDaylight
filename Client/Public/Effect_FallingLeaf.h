...
class CEffect_FallingLeaf final : public CEffect {
	struct DirKeyFrame {
		_float	fDirTime = 0.f;
...
public:
	static CEffect_FallingLeaf* const Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork, const _tchar* const& pFilePath = nullptr);
	virtual CEffect* const Clone(void* const& pArg = nullptr) override;
private:
	explicit CEffect_FallingLeaf(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork);
	explicit CEffect_FallingLeaf(const CEffect_FallingLeaf& rhs);
	virtual ~CEffect_FallingLeaf(void) override = default;
