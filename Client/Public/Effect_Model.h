...
class CEffect_Model final : public CObject {
	struct EffectModel {
...
public:
	static CEffect_Model* const Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork);
	virtual CObject* const Clone(void* const& pArg = nullptr) override;
private:
	explicit CEffect_Model(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork);
	explicit CEffect_Model(const CEffect_Model& rhs);
	virtual ~CEffect_Model(void) override = default;
...
public:
	...
	const HRESULT Create_Effect(EffectModel sEffect);
	void SaveEffect(const _tchar* const& pFilePath);
