...
class CEffect_Texture final : public CObject {
	struct DirKeyFrame {
		_float	fDirTime = 0.f;
...
public:
	static CEffect_Texture* const Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork, const _tchar* const& pFilePath = nullptr);
	virtual CObject* const Clone(void* const& pArg = nullptr) override;
private:
	explicit CEffect_Texture(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork);
	explicit CEffect_Texture(const CEffect_Texture& rhs);
	virtual ~CEffect_Texture(void) override = default;
...
public:
	...
	const HRESULT Create_Effect(EffectTex sEffect);
