...
class CEffect_DrumFire final : public CObject {
	struct DirKeyFrame {
		_float	fDirTime = 0.f;
...
public:
	static CEffect_DrumFire* const Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork, const _tchar* const& pFilePath = nullptr);
	virtual CObject* const Clone(void* const& pArg = nullptr) override;
private:
	explicit CEffect_DrumFire(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork);
	explicit CEffect_DrumFire(const CEffect_DrumFire& rhs);
	virtual ~CEffect_DrumFire(void) override = default;
