...
class CEffect_DrumFireEtc final : public CObject {
	struct DirKeyFrame {
		_float	fDirTime = 0.f;
...
public:
	static CEffect_DrumFireEtc* const Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork, const _tchar* const& pFilePath = nullptr);
	virtual CObject* const Clone(void* const& pArg = nullptr) override;
private:
	explicit CEffect_DrumFireEtc(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork);
	explicit CEffect_DrumFireEtc(const CEffect_DrumFireEtc& rhs);
	virtual ~CEffect_DrumFireEtc(void) override = default;
