...
class CEffect_HookSmoke final : public CEffect {
	struct DirKeyFrame {
		_float	fDirTime = 0.f;
...
public:
	static CEffect_HookSmoke* const Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork, const _tchar* const& pFilePath = nullptr);
	virtual CEffect* const Clone(void* const& pArg = nullptr) override;
private:
	explicit CEffect_HookSmoke(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork);
	explicit CEffect_HookSmoke(const CEffect_HookSmoke& rhs);
	virtual ~CEffect_HookSmoke(void) override = default;
