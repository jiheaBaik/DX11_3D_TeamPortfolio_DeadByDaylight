...
class CEffect_CamperHookDeath final : public CObject {
	struct DirKeyFrame {
		_float	fDirTime = 0.f;
...
public:
	static CEffect_CamperHookDeath* const Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork, const _tchar* const& pFilePath = nullptr);
	virtual CObject* const Clone(void* const& pArg = nullptr) override;
private:
	explicit CEffect_CamperHookDeath(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork);
	explicit CEffect_CamperHookDeath(const CEffect_CamperHookDeath& rhs);
	virtual ~CEffect_CamperHookDeath(void) override = default;
