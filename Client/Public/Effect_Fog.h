...
class CEffect_Fog final : public CEffect {
	struct DirKeyFrame {
		_float	fDirTime = 0.f;
...
public:
	static CEffect_Fog* const Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork, const _tchar* const& pFilePath = nullptr);
	virtual CEffect* const Clone(void* const& pArg = nullptr) override;
private:
	explicit CEffect_Fog(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, class CNetwork* const& pNetwork);
	explicit CEffect_Fog(const CEffect_Fog& rhs);
	virtual ~CEffect_Fog(void) override = default;
...
private:
	...
	_float	m_SetEndTime = 0.f;
	class CTarget* m_pTarget_Depth = nullptr;

	_float4	m_fColorAlpha = {0.f,0.f,0.f,0.f};
};
END_NAMESPACE
